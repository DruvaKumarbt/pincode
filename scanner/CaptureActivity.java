...
import com.smart.ticketing.parkingqr.Database;
import com.smart.ticketing.parkingqr.MainActivity;
import com.smart.ticketing.parkingqr.ViewCostActivity;
...
//        updateDatabse(QRContent);
    ...
    public void updateDatabse(String content) {
        String[] temp = content.split("\\#");

        Database database = new Database(this);
        SQLiteDatabase db = database.getWritableDatabase();
        Log.i(TAG, "content: "+temp[0]);

//        Cursor c = db.rawQuery("select * from Disease where carno = '" + temp[0] + "' and slot='" + temp[1] + "'", null);
        Cursor c = db.rawQuery("select * from Disease where carno = '" + temp[0] + "'", null);
        if (c.getCount() > 0) {
            c.moveToFirst();
            String startTime = c.getString(c.getColumnIndex("startTime"));
            if (startTime.equals("0")) {
                ContentValues cv = new ContentValues();
                cv.put("startTime", System.currentTimeMillis() + "");

                db.update("Disease", cv, "carno = '" + temp[0] + "' and slot='" + temp[1] + "'", null);
                Utils.showToast(getApplicationContext(), "Start time updated...");
//                sendSMS(temp[1], true);
            } else {

                long stTime = Long.parseLong(startTime);
                long endTime = System.currentTimeMillis();

                long diff = (endTime - stTime) / 1000;
                float amount = (float) (diff * 0.2);
                Utils.showToast(getApplicationContext(), "total difference is : "+diff);
                db.delete("Disease", "carno = '" + temp[0] + "'", null);
                String costinfo = "Total cost for "+diff +" sec is "+amount;
                Intent intent = new Intent(CaptureActivity.this, ViewCostActivity.class);
                intent.putExtra("data", costinfo);
                intent.putExtra("slot", temp[0]);
                startActivity(intent);
//                sendSMS(temp[1], false);
                Date startDate = new Date();
                startDate.setTime(stTime);
                Date endDate = new Date();
                endDate.setTime(endTime);
                writeToFile("\n"+getSpacingCount(content)+startDate.toString()+"       "+endDate.toString()+"        "+amount);
            }
        }else{
            long startTime = System.currentTimeMillis();
            Utils.showToast(getApplicationContext(), "Inserting and updatating start value");
            ContentValues cv1 = new ContentValues();
            cv1.put("carno", content);
            cv1.put("startTime", startTime+"");
            db.insert("Disease", null, cv1);
//            writeToFile(content+"\t"+startTime);
        }
    }
    ...
    public void sendSMS(String slot, boolean isOpen){
        SmsManager sms = SmsManager.getDefault();

        if(slot.equals("S1")){
            if(isOpen){
                sms.sendTextMessage(MainActivity.hardwarephone, null, "a", null, null);
            }else{
                sms.sendTextMessage(MainActivity.hardwarephone, null, "b", null, null);
            }
        }else if(slot.equals("S2")){
            if(isOpen){
                sms.sendTextMessage(MainActivity.hardwarephone, null, "c", null, null);
            }else{
                sms.sendTextMessage(MainActivity.hardwarephone, null, "d", null, null);
            }
        }else if(slot.equals("S3")){
            if(isOpen){
                sms.sendTextMessage(MainActivity.hardwarephone, null, "e", null, null);
            }else{
                sms.sendTextMessage(MainActivity.hardwarephone, null, "f", null, null);
            }
        }
    }
